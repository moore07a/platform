 ...
 */
function syncDailyFeedConsumption(PDO $pdo, int $farmId, ?int $oldTransactionId, int $feedItemId, float $quantity, string $date, string $feedCategory, ?int $userId): ?int {
    if ($oldTransactionId) {
    ...
    $itemStmt = $pdo->prepare('SELECT id, current_stock, unit, farm_type FROM stock_items WHERE id = ? AND farm_id = ? AND feed_category = ? AND is_active = 1 FOR UPDATE');
