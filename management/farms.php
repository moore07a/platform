...
function saveFarmLogoUpload(?array $file, int $farmId, ?string $existing = null): ?string {
    if (empty($file['tmp_name'])) return $existing;
    if ($file['error'] !== UPLOAD_ERR_OK || $file['size'] > 2 * 1024 * 1024) throw new RuntimeException('Logo upload must be an image smaller than 2 MB.');

    $imageInfo = @getimagesize($file['tmp_name']);
    $imageType = $imageInfo[2] ?? null;
    $extensions = [IMAGETYPE_JPEG => 'jpg', IMAGETYPE_PNG => 'png', IMAGETYPE_WEBP => 'webp'];
    if (!isset($extensions[$imageType])) throw new RuntimeException('Logo must be a JPG, PNG, or WebP image.');
    ...
    $filename = $farmId . '-' . bin2hex(random_bytes(12)) . '.' . $extensions[$imageType];
...
function saveModulesAndRoles(PDO $pdo, int $farmId, int $ownerId, array $roles): void {
    ...
    foreach ($roles as $role) $roleStmt->execute([$ownerId, $role]);
}
...
function deleteFarmData(PDO $pdo, int $farmId): void {
    ...
    foreach (['customer_ledger_entries', 'stock_transactions', 'stock_batches', 'layer_daily_records', 'broiler_daily_records', 'ruminant_daily_records', 'farm_expenses', 'profit_loss_summary', 'sales_records', 'production_cycles', 'stock_items', 'inventory_categories', 'users'] as $table) {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    elseif (isset($_POST['update_farm']) && $password !== '' && strlen($password) < FARM_OWNER_MIN_PASSWORD_LENGTH) $error = 'A replacement password must be at least ' . FARM_OWNER_MIN_PASSWORD_LENGTH . ' characters.';
        ...
        if (isset($_POST['create_farm'])) {
            ...
            $farmId = (int)$pdo->lastInsertId(); $logoPath = saveFarmLogoUpload($_FILES['logo'] ?? null, $farmId);
            ...
            $ownerStmt = $pdo->prepare("SELECT u.id FROM users u INNER JOIN user_roles ur ON ur.user_id = u.id INNER JOIN roles r ON r.id = ur.role_id WHERE u.farm_id = ? AND r.code = 'farm_admin' ORDER BY u.id LIMIT 1"); $ownerStmt->execute([$farmId]); $ownerId = (int)$ownerStmt->fetchColumn();
            if (!$ownerId) throw new RuntimeException('This farm has no admin account to update.');
            $logoPath = saveFarmLogoUpload($_FILES['logo'] ?? null, $farmId, $farm['logo_path']);
    ...
    } catch (Throwable $e) { if ($pdo->inTransaction()) $pdo->rollBack(); $error = $e instanceof RuntimeException ? $e->getMessage() : 'Unable to save this farm. Its workspace ID or owner username may already exist.'; }
}

$editFarm = null; $editOwner = null; $editModules = [];
if (isset($_GET['edit'])) { $editFarm = editableFarm($pdo, validFarmId($_GET['edit'])); if ($editFarm) { $stmt = $pdo->prepare("SELECT u.* FROM users u INNER JOIN user_roles ur ON ur.user_id = u.id INNER JOIN roles r ON r.id = ur.role_id WHERE u.farm_id = ? AND r.code = 'farm_admin' ORDER BY u.id LIMIT 1"); $stmt->execute([$editFarm['id']]); $editOwner = $stmt->fetch(PDO::FETCH_ASSOC); $moduleStmt = $pdo->prepare('SELECT module_code FROM farm_modules WHERE farm_id = ? AND is_enabled = 1'); $moduleStmt->execute([$editFarm['id']]); $editModules = $moduleStmt->fetchAll(PDO::FETCH_COLUMN); } }
