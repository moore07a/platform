...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['delete_transaction']) && $isOwner) {
    ...
    try {
        ...
        $transStmt = $pdo->prepare("SELECT * FROM stock_transactions WHERE id = ? AND farm_id = ? AND farm_type = 'ruminant' FOR UPDATE");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['edit_transaction']) && $isOwner) {
    ...
    try {
        ...
        // Revert stock impact on the original item
        ...
        // If the item changes, use the selected item's current stock; otherwise use reverted stock
